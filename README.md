...
| `lan_message(peer, content, session?)` | 发消息；`peer` 支持设备名、设备 ID 或 `ip:port`；`session` 可选，填 `lan_peers` 里看到的会话 ID 即可精确投递给那个会话，省略则投给设备的默认主 Agent |

消息进入目标 Agent 的收件箱并唤醒它开始新对话。空消息会被拒绝，超过 4000 字自动截断，对端离线会立即失败（消息不排队）。

**界面**：设置页「LAN 协作」的节点列表会显示每台设备的会话数，发消息卡可下拉选择目标会话，通信消息记录标注会话标题；装有 dsh-plugin-vscode-sidebar 时侧边栏有同名标签页；装有 dsh-notifacation-frame 时收到消息会推送通知。
